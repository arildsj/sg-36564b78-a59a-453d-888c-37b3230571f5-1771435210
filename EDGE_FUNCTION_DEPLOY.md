...
## Funksjon: inbound-message

### Oppdatert kode (deploy denne):
          ...
          if (rules && rules.length > 0) {
            for (const rule of rules) {
              ...
              if (rule.rule_type === "keyword") {
                ...
                matches = from_number.includes(pattern) || pattern.includes(from_number);
...
1. Gå til: https://supabase.com/dashboard/project/vrbzjgvdlnkffwjhbvkh/functions
2. Finn `inbound-message` funksjonen
3. Klikk på funksjonsnavnet
4. Klikk "Edit" eller "Redeploy"
5. Erstatt eksisterende kode med koden over
6. Klikk "Deploy"

### Via Supabase CLI (krever Docker):
```bash
npx supabase functions deploy inbound-message --project-ref vrbzjgvdlnkffwjhbvkh
```

## Hva er nytt i denne versjonen:

✅ **Støtte for `target_group_id` parameter**
- Simulering kan nå sende meldinger til spesifikke grupper
- Overstyrerer routing-regler når satt

✅ **Ny regeltype: "sender" (Avsender)**
- Matcher meldinger basert på avsendernummer
- Støtter både numeriske (+4712345678) og alfanumeriske (BANK) avsendere
- Pattern matching: `from_number.includes(pattern) || pattern.includes(from_number)`

✅ **Forbedret fallback-logikk**
- Bruker gateway's fallback_group_id når ingen regler matcher
- Bedre feilhåndtering for manglende målgrupper

## Testing:

Etter deployment, test med:
1. **Simulering med spesifikk gruppe:** Velg målgruppe i simulate-siden
2. **Avsender-regel:** Opprett regel med regeltype "Avsender" og test med matching nummer
3. **Fallback:** Send melding uten matching regler for å teste fallback-gruppe

## Support:

Hvis deployment feiler, kontakt teamet eller sjekk Supabase logs i Dashboard.