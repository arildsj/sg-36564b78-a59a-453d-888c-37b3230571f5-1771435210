-- SeMSe + FairGateway: Row Level Security Policies (FIXED v2)
-- Removed OLD reference from policy
...
-- Check if user is admin of group or any ancestor
CREATE OR REPLACE FUNCTION public.is_group_admin_of_subtree(p_target_group_id UUID)
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 
    ...
    WHERE gm.user_id = auth.uid()
      ...
      AND g_target.path @> g_admin.path
...
CREATE POLICY "Tenant admins can update own tenant"
  ...
  USING (id = public.user_tenant_id() AND public.is_tenant_admin());
...
CREATE POLICY "Users can view groups they belong to"
  ...
  USING (
    ...
    AND (
      ...
      OR id IN (SELECT public.user_group_ids())
      OR public.is_group_admin_of_subtree(id)
    )
  );
...
CREATE POLICY "Group admins can update their groups"
  ...
  USING (
    tenant_id = public.user_tenant_id() 
    AND public.is_group_admin_of_subtree(id)
  );
...
CREATE POLICY "Tenant admins can manage all memberships"
  ...
  USING (
    group_id IN (
      SELECT id FROM groups WHERE tenant_id = public.user_tenant_id()
    )
...
CREATE POLICY "Group admins can manage their group memberships"
  ON group_memberships FOR ALL
  USING (public.is_group_admin_of_subtree(group_id));
...
CREATE POLICY "Users can manage phones for accessible contacts"
  ...
  USING (
    contact_id IN (
      SELECT id FROM contacts WHERE tenant_id = public.user_tenant_id()
    )
  );
...
-- ============================================================================
-- WHITELISTED NUMBER GROUP LINKS
-- ============================================================================
CREATE POLICY "Users can view whitelist links for their groups"
  ...
  USING (
    ...
    OR whitelisted_number_id IN (
      SELECT id FROM whitelisted_numbers WHERE tenant_id = public.user_tenant_id()
    )
  );
...
-- ============================================================================
-- GATEWAY FALLBACK INBOXES
-- ============================================================================
CREATE POLICY "Users can view fallback inboxes"
  ON gateway_fallback_inboxes FOR SELECT
  USING (
    gateway_id IN (
      SELECT id FROM gateways WHERE tenant_id = public.user_tenant_id()
    )
  );
...
CREATE POLICY "Users can view messages in their groups"
  ...
  USING (
    tenant_id = public.user_tenant_id()
    AND resolved_group_id IN (SELECT public.user_group_ids())
  );
...
CREATE POLICY "Group admins can manage group auto replies"
  ...
  USING (
    ...
    AND public.is_group_admin_of_subtree(group_id)
  );
...
CREATE POLICY "System can manage bulk recipients"
  ...
  USING (
    campaign_id IN (
      SELECT id FROM bulk_campaigns WHERE tenant_id = public.user_tenant_id()
    )
  );
