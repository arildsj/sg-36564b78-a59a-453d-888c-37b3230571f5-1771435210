...
BEGIN
  IF TG_OP = 'INSERT' THEN
    PERFORM log_audit_event(
      NEW.tenant_id,
      NEW.id, -- Use NEW.id for INSERT since it's the user being created
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF (OLD.role IS DISTINCT FROM NEW.role) OR 
      ...
      PERFORM log_audit_event(
        NEW.tenant_id,
        NEW.id, -- User updating themselves
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    PERFORM log_audit_event(
      NEW.tenant_id,
      NEW.tenant_id, -- System/admin action
...
BEGIN
  ...
  WHERE a.entity_type = p_entity_type
    ...
    AND a.tenant_id = (SELECT tenant_id FROM user_profiles WHERE id = u.id LIMIT 1)
